export { default as withRouter } from '../hoc/withRouter';
export { default as Board } from "./Dashboard/Board/Board";
export { default as Dashboard } from "./Dashboard/Dashboard";
export { default as Icon } from "./Dashboard/Icon/Icon";
export { default as BoardNavbar } from "./Dashboard/Shared/Navbar";
export { default as Sidebar } from "./Dashboard/Shared/Sidebar";
export { default as Template } from "./Dashboard/Template/Template";
export { default as Faq } from "./Faq/Faq";
export { default as Hero } from "./Hero/Hero";
export { default as IconPack } from "./IconPack/IconPack";
export { default as Icons } from "./Icons/Icons";
export { default as Pricing } from "./Pricing/Pricing";
export { default as Banner } from "./Shared/Banner/Banner";
export { default as Footer } from "./Shared/Footer/Footer";
export { default as Layout } from "./Shared/Layout/Layout";
export { default as Navbar } from "./Shared/Navbar/Navbar";
export { default as SingleIcon } from "./SingleIcon/SingleIcon";
export { default as CodedBlocks } from "./Templates/CodedBlocks";
export { default as CodedTemplates } from "./Templates/CodedTemplates";
export { default as EmailTemplates } from "./Templates/EmailTemplates";
export { default as AccountSetting } from "./User/AccountSetting";
export { default as CheckoutAfterLogin } from "./User/CheckoutAfterLogin";
export { default as CheckoutBeforeLogin } from "./User/CheckoutBeforeLogin";
export { default as ForgotPassword } from "./User/ForgotPassword";
export { default as Login } from "./User/Login";
export { default as NewPassword } from "./User/NewPassword";
export { default as Otp } from "./User/Otp";
export { default as Register } from "./User/Register";
export { default as ResetPasswordSucess } from "./User/ResetPasswordSucess";
export { default as BillingBasic } from "./User/Subscription/BillingBasic";
export { default as BillingLifetime } from "./User/Subscription/BillingLifetime";
export { default as BillingYearly } from "./User/Subscription/BillingYearly";



